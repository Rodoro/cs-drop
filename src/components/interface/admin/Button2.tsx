    ...
    ({ children, className, ...props }, ref: ForwardedRef<HTMLButtonElement>) => {
        return (
            <button
                ...
                className={`transition-all duration-300 ease-in-out w-full min-h-[56px] py-[12px] px-[9px] rounded-2xl bg-[#11162E4D]  text-[#F9FAFB] hover:bg-[#11162E] hover:shadow-[4px_4px_34px_0_rgba(139,50,252,0.2)] ${className}`}
                {...props}
            >
                {children}
            </button>
