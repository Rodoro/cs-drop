...
const InputSearch: React.FC<InputProps> = ({ value, setValue, ...props }: any) => {
    return (
        <div className="relative w-full sm:w-[482px] p-[1px] rounded-[0.9375rem]" { ...props } style={{background: "linear-gradient(150deg,rgba(68, 72, 113, 1),rgba(39, 49, 87, 1))"}}>
            ...
            <input value={value} onChange={(e) => setValue(e.target.value)} className="ps-12 flex items-center gap-3 py-4 px-5 w-full sm:w-[480px] h-[3.125rem] rounded-[0.9375rem] bg-[#263056] focus-visible:outline-none" placeholder="Search for cases" />
        </div>
    )
}
